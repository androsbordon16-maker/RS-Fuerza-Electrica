          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {datos.rect_rows.map((row,i) => (
                <tr key={i}>
                  {['al','el'].map((f,j) => j===0 ? (
                    <td key={f} className="pr-1 pb-1"><input className="border border-gray-300 rounded px-1.5 py-1 text-xs w-16 focus:outline-none" value={row[f]} onChange={e=>updArr('rect_rows',i,f,e.target.value)} disabled={!puedeEditar} /></td>
                  ) : null)}
                  ...
                  <td className="pr-1 pb-1">
                    <div className="flex items-center gap-0.5 rounded px-1.5 py-1" style={{background:'#bae6fd'}}>
                      <span className="text-blue-800 font-bold text-xs whitespace-nowrap">RECT.=</span>
                      <input className="border-0 bg-transparent text-xs w-10 focus:outline-none text-blue-900 font-medium" value={row.rect_izq||''} onChange={e=>updArr('rect_rows',i,'rect_izq',e.target.value)} placeholder="1-2" disabled={!puedeEditar} />
                    </div>
                  </td>
                  <td className="pr-1 pb-1">
                    <div className="flex items-center gap-0.5 rounded px-1.5 py-1" style={{background:'#bae6fd'}}>
                      <span className="text-blue-800 font-bold text-xs whitespace-nowrap">AMP=</span>
                      <input className="border-0 bg-transparent text-xs w-12 focus:outline-none text-blue-900 font-medium" value={row.amp_izq||''} onChange={e=>updArr('rect_rows',i,'amp_izq',e.target.value)} placeholder="2X40" disabled={!puedeEditar} />
                    </div>
                  </td>
                  <td className="pr-1 pb-1">
                    <div className="flex items-center gap-0.5 rounded px-1.5 py-1" style={{background:'#bae6fd'}}>
                      <span className="text-blue-800 font-bold text-xs whitespace-nowrap">RECT.=</span>
                      <input className="border-0 bg-transparent text-xs w-10 focus:outline-none text-blue-900 font-medium" value={row.rect_der||''} onChange={e=>updArr('rect_rows',i,'rect_der',e.target.value)} placeholder="3-4" disabled={!puedeEditar} />
                    </div>
                  </td>
                  <td className="pr-1 pb-1">
                    <div className="flex items-center gap-0.5 rounded px-1.5 py-1" style={{background:'#bae6fd'}}>
                      <span className="text-blue-800 font-bold text-xs whitespace-nowrap">AMP=</span>
                      <input className="border-0 bg-transparent text-xs w-12 focus:outline-none text-blue-900 font-medium" value={row.amp_der||''} onChange={e=>updArr('rect_rows',i,'amp_der',e.target.value)} placeholder="2X40" disabled={!puedeEditar} />
                    </div>
                  </td>
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">{enc.planta||'Sin nombre'} {!puedeEditar && '· Solo lectura'}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          {!esNuevo && perfil?.rol === 'admin' && (
            <select className="border border-gray-300 rounded-lg px-2 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={estado} onChange={e=>guardar(e.target.value)}>
          ...
          {!esNuevo && (
            <button onClick={()=>nav(`/reporte/${id}/excel`)} className="bg-blue-600 text-white text-sm rounded-lg px-3 py-1.5 hover:bg-blue-700">Generar Excel</button>
          )}
        </div>
      </div>
